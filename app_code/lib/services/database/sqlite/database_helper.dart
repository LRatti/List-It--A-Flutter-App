...
/// Helper class to manage SQLite database connection and initialization
class DatabaseHelper {
  ...
  static Future<void> _upgradeDb(Database db, int oldVersion, int newVersion) async {
    ...
    if (oldVersion < 5) {
      // Add is_default column to supermarket table
      try {
        await db.execute('ALTER TABLE supermarket ADD COLUMN is_default INTEGER NOT NULL DEFAULT 0');
      } catch (e) {
        // Column might already exist, ignore
      }
    }
