...
class _RecordingAuthRepository implements AuthRepository {
	...
	@override
	Future<User?> linkAnonymousWithEmailPassword(String email, String password, String username) async => null;
	@override
...
void main() {
	...
	setUpAll(() {
		registerFallbackValue(MaterialPageRoute(builder: (context) => const SizedBox(), settings: const RouteSettings(name: '/')));
	});

	Widget _buildApp({required Widget child, NavigatorObserver? observer}) {
	...
	testWidgets('updates email and navigates to verification with session set', (tester) async {
		...
		await tester.pumpWidget(_buildApp(
			...
			child: SettingsScreen(userManager: _FakeUserManager(user), authRepository: repo),
		));
